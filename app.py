...
# -----------------------------
# Train model (FAIL-SAFE)
# -----------------------------
def train_model_safe(X, y):
    try:
        ...
        fallback.fit(X, y)
        return fallback, "DummyClassifier (fallback)"

model, model_used = train_model_safe(X_train, y_train)
